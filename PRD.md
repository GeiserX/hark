...
Beyond batch and pipe usage, Hark can run **interactively** — a minimal terminal UI that shows the live transcript and accepts pause/resume/stop from the keyboard, plus user-defined shortcut keys that run a configured command against the live transcript — and as an on-demand **remote-control agent** that external programs (including browser userscripts) drive over a local HTTP/JSON API to decide when, what, and where to record. The two combine: `hark --interactive --remote-control` is a long-lived TUI agent that sits idle in a terminal until the browser starts a meeting, then shows the live transcript and accepts the same keyboard controls (including shortcuts) for the current session.
...
| Make live capture observable and externally controllable | Starting a capture prints the resolved engine/model/source/format/outputs to stderr; `--interactive` exposes pause/resume, mic mute/unmute, transcript yank-to-clipboard, stop, and user-defined `shortcut.<k>` commands (live transcript path in `$HARK_TRANSCRIPT`) at the keyboard without stalling capture; `--remote-control` lets an external script start/stop/pause/query a recording via the documented HTTP API with a round-trip < 200 ms (except `POST /start`, which answers when the capture is open); combined (`--interactive --remote-control`) the TUI and the API share one session — the browser starts/stops meetings, the keyboard recaps them |
...
| 12 | Live transcription | P1 | During live capture, emit the transcript incrementally — as close to runtime as possible — by segmenting the stream on natural pauses and transcribing each segment as it completes, or continuously with `--live-streaming`, which feeds the whole capture through a streaming recognizer so a line's first words appear about 2 s after they are spoken. |
...
| 22 | Interactive mode | P1 | `--interactive`: a minimal terminal UI for live capture — a status header, the live transcript, and single-key controls (**space** = pause/resume, **m** = mute/unmute the mic, **y** = yank the transcript to the clipboard, **Enter** = finish; Ctrl-C also stops). The live transcript is always shown on screen; naming `-t FILE`/`-a FILE` concurrently persists the transcript/audio to those files. **Pause excludes the paused interval** from both audio and transcript (a true gap, so output is shorter than wall-clock); **mute** silences only the microphone (timeline preserved — distinct from pause), and **yank** copies the full session transcript so far to the system clipboard. User-defined shortcut keys (`shortcut.<k>`, Feature 24) run a configured command against that same live transcript. Requires a controlling TTY; incompatible with stdout (`-`) streaming. **Alone** it captures immediately and Enter/Ctrl-C exit; **with `--remote-control`** (Feature 25) capture is deferred to `POST /start` and Enter only stops the session. See §6.9. |
| 23 | Remote-control agent | P1 | `hark --remote-control [ADDR]` runs Hark as a control agent (no immediate capture) exposing an **HTTP/JSON API over TCP** — bound to loopback by default, or to an explicit `interface:port`. External scripts choose **when** (start/stop/pause/resume), **what** (sources/format/engine/model), and **where** (output paths resolved under the working `directory`). Documented wire protocol (no bundled client) plus a Tampermonkey Google-Meet reference userscript. **Alone** it is headless (`brew services` stays this); **with `--interactive`** (Feature 25) the same agent also drives the TUI. See §6.10. |
| 24 | Interactive shortcuts | P1 | In `--interactive` (alone or combined with `--remote-control`), user-defined single keys run a pre-configured shell command. Bindings are scalar config keys `shortcut.<k>` (e.g. `shortcut.w`). On keypress Hark snapshots the live caption log to a temp file, sets `$HARK_TRANSCRIPT` to that path on the child, and runs `/bin/sh -c <command>` detached (child stdin/stdout/stderr discarded; the command handles its own UX, e.g. piping to `pbcopy`). Built-in keys (space, **m**, **y**, Enter) cannot be rebound. Not exposed on the remote-control HTTP API. In combined mode the log is kept after `POST /stop` / Enter until the next `POST /start`, so a recap still works after the call. See §6.9. |
| 25 | Combined interactive agent | P1 | `hark --interactive --remote-control [ADDR]` is a long-lived TUI agent: the HTTP API and the keyboard share one session. Idle (no capture) until `POST /start`; live captions while recording; **Enter** stops the session and returns to idle; **Ctrl-C** quits the process. Launch flags are session defaults. Headless `brew services` is unchanged (`--remote-control` only). See §6.9, §6.10. |
| 26 | Existing-output protection | P1 | No output is ever clobbered silently. Before capture starts, Hark checks every file the run would write (audio, transcript, `--split` chunk set) and applies `--if-exists ask\|error\|overwrite\|unique` (default `ask`; also `$HARK_IF_EXISTS` / config `if-exists`). On a terminal `ask` offers overwrite / a numbered name / cancel; without a terminal it refuses (exit 73). The invocation's outputs are one artifact set: a single decision covers all of them and `unique` picks one suffix free for each, so an audio/transcript pair stays aligned (`rec-1.m4a` + `rec-1.txt`). The remote-control agent (§6.10) defaults to `unique` so a session never blocks or overwrites. See §6.1. |
...
- **Multi-channel mapping** beyond the two-source L/R layout of `--tracks stereo` (§6.7d): separate *files* per source, or more than two mapped sources.
...
As a **developer in a live meeting**, I want to press a custom key and run my own summarizer on the transcript so far, so that I can recap without stopping the recording.
- Acceptance Criteria:
  - [ ] `hark config set shortcut.w 'fabric-ai -p summarize_meeting "$HARK_TRANSCRIPT" | pbcopy'` then `hark --interactive --mix` — pressing **w** snapshots the live captions to a temp file, sets `$HARK_TRANSCRIPT` to that path, and launches `/bin/sh -c` detached; capture and the key reader continue
  - [ ] The child's stdin/stdout/stderr are discarded (a pipeline *inside* the command, e.g. `| pbcopy`, still works); the child does not steal the TTY
  - [ ] Pressing **w** before anything is transcribed prints a brief "nothing to pass yet" notice and does not spawn
  - [ ] A one-line stderr ack (`hark: shortcut.w`) is printed when the command is launched; command output is never printed by Hark
  - [ ] `shortcut.m` and `shortcut.y` are rejected (reserved); `hark config show` lists `shortcut.*` keys; `hark config unset shortcut.w` removes the binding
  - [ ] The temp file is deleted when the command exits; a second press while the first command is still running spawns another process
  - [ ] Shortcuts are interactive-only (the remote-control API does not expose them)

### US14 — One terminal, many meetings
As a **developer**, I want Hark left running in a terminal while the browser starts and stops meetings, so that I can walk back and recap the current (or just-ended) call without relaunching.
- Acceptance Criteria:
  - [ ] `hark --interactive --remote-control --system --mix` starts the TUI and binds the API; it does **not** capture until `POST /start`; stderr says it is waiting
  - [ ] While idle, **Enter** is a no-op; **y** / shortcuts print "nothing to pass/copy yet"; **Ctrl-C** exits
  - [ ] `POST /start` (e.g. the Meet userscript) begins capture; live captions stream on stdout; **space** / **m** / **y** / `shortcut.<k>` work; `POST /mute` still mirrors Meet
  - [ ] **Enter** and `POST /stop` both finalise the session and return to idle; the caption log is **kept** so **y** / shortcuts still recap that meeting
  - [ ] The next `POST /start` clears the log (meetings do not mix); a `POST /start` while already recording returns `409`
  - [ ] **Ctrl-C** (or SIGTERM) finalises if recording, then exits the process
  - [ ] `hark --interactive` alone is unchanged (captures immediately; Enter exits)
  - [ ] `hark --remote-control` alone is unchanged (headless; `brew services` stays this)
  - [ ] Combined mode still requires a controlling TTY (clear usage error otherwise)
...
- `--if-exists ask|error|overwrite|unique` : what to do when a file the run would write already exists (default `ask`; also `$HARK_IF_EXISTS` or config `if-exists`). Checked **before** capture starts — before permission prompts and model loading — so nothing is destroyed and no long recording is lost to a late failure. `ask` prompts on a terminal (`[o]verwrite` / `[u]nique` / `[c]ancel`) and behaves like `error` without one (cron, pipes, the agent); `error` refuses with exit 73; `overwrite` replaces (and for `--split` removes a previous run's stale `NAME_NNN` chunks first); `unique` writes the next free numbered name (`rec.m4a` → `rec-1.m4a`). The invocation's outputs form **one artifact set**: a single decision covers all of them, and `unique` picks one suffix that is free for every output so an audio/transcript pair keeps matching names. Outputs on `-` (stdout) are never affected. Appending is left to the shell (`hark -t - >> notes.txt`) — it works for text and is impossible for most audio containers, so there is no `append` mode.
- An output that is also the input (`-i rec.wav -a rec.wav`), or `-a` and `-t` naming the same file, is a usage error.
...
- `--interactive` : run live capture in a minimal terminal UI (status header + live transcript + single-key controls: **space** pause/resume, **m** mute/unmute the mic, **y** yank the transcript to the clipboard, **Enter** finish; Ctrl-C also stops; plus any `shortcut.<k>` bindings from config). Pause omits the paused interval from the outputs; mute silences only the mic (timeline preserved); yank copies the transcript so far to the system clipboard; a shortcut snapshots that transcript and runs the bound command detached. Requires a controlling TTY; mutually exclusive with `-` (stdout) outputs. **Alone** captures immediately and Enter/Ctrl-C exit; **with `--remote-control`** it does not capture until `POST /start` and Enter only stops the session (Feature 25). (Modal flag — not a persisted config key; the bindings themselves are persisted as `shortcut.<k>`.)
- `--remote-control [[host:]port]` : start Hark as a control **agent** instead of capturing immediately, serving an HTTP/JSON API over TCP. The value is **optional**: omitted, it binds **loopback** on the configured `remote-control-port` (default `8473`); a bare port or an empty host binds loopback on that port (e.g. `8473` or `:8473`); `0.0.0.0:8473` or a specific IPv4 binds elsewhere. Any other capture/transcription flags given at launch become the per-session **defaults** for recordings started via the API. **With `--interactive`** the same agent also runs the TUI (Feature 25); **alone** it is headless. Non-loopback binds require `$HARK_REMOTE_TOKEN`. (Agent **mode** is modal — not persisted; the **port** it binds is the persisted `remote-control-port` config key, so a `brew services`-managed agent can launch without an explicit address.)
...
hark --interactive --remote-control --system --mix  # TUI agent; browser POST /start, press w to recap
```
...
- Keys (kebab-case): `engine`, `model`, `language`, `translate`, `device`, `directory`, `capture-backend`, `rate`, `bits`, `channels`, `silence-threshold`, `vad`, `vad-threshold`, `segment-pause`, `segment-window`, `live-streaming`, `gain`, `speakers`, `speaker-mode`, `speaker-labels`, `diarize-engine`, `max-speakers`, `speaker-threshold`, `remote-control-port`. Values are type-checked (e.g. `translate`/`vad`/`gain`/`speakers` boolean; `silence-threshold` negative; `engine` a known engine; `directory` an existing directory; `vad-threshold`/`speaker-threshold` in 0–1; `segment-pause` in 0–5 s and `segment-window` in 1–60 s, the window greater than the pause; `live-streaming` boolean; `remote-control-port` an integer port `1–65535`); unknown keys are rejected. **Exception — interactive shortcuts:** keys matching `shortcut.<k>` (`k` = one letter or digit, case-insensitive) are shortcut bindings, not members of the typed settings registry; `hark config set shortcut.w '…'` / `unset shortcut.w` / `show` accept them (see §6.9). `hark config show` lists every setting with its value, source, and a one-line description. Values beginning with `-` are taken verbatim (e.g. `hark config set silence-threshold -40`).
...
| Live streaming | `--live-streaming`/`--no-live-streaming` | `$HARK_LIVE_STREAMING` | `live-streaming` | `false` |
...
- For live capture, transcription should run as close to runtime as possible: the stream is segmented on natural pauses (with a maximum-window cap) and each segment is transcribed as it completes, appending to the destination. `--live-streaming` transcribes continuously instead of one window per pause. Hark feeds the whole capture through the Nemotron multilingual streaming recognizer and cuts its growing token list into lines on the same `--segment-pause`/`--segment-window` rules, so a line's first words appear about 2 s after they are spoken instead of 9 to 12 s, and the open line grows in place until a pause closes it. Apple Silicon, English/Spanish/French/Italian/Portuguese/German in one 583 MB model, off by default. The streaming path picks its own recognizer and does its own segmenting, so `--engine`, `--vad`, `--vad-threshold` and `--gain` do not apply, and Hark names the ones the user set deliberately on its standard error (there is no remote-control field for the notice). `--silence-threshold` still applies, because the same run uses it for `--split silence:<n>`. It cannot `--translate`, `--diarize-engine offline` ignores it, and `-i FILE` with it is a usage error. Wherever it cannot run, Hark prints why and the segmented path runs unchanged. Batch (transcribe-at-end) is the minimum acceptable behaviour for v1. **Segment boundaries are determined by voice-activity detection (VAD) when available** (Silero via FluidAudio — far more stable than a raw amplitude threshold), falling back to the `--silence-threshold` amplitude heuristic when VAD models are absent; see §6.7. When `--speakers` is active, the same segmentation feeds the speaker pipeline so each emitted segment carries a speaker label.
...
**c) Combined.** With `--speakers` (auto/acoustic) and two sources, Hark attributes the mic side as `You` (deterministic) and diarizes the system side, yielding `You` plus `Speaker 1/2…` for the remote participants in one transcript — live (streaming) or as an accurate offline pass at stop.

`You` needs the two sources to still be separate, which on a file means **two channels**: `-i FILE --speakers --speaker-mode source` reads a two-channel recording as the mic on channel 1 (left) and the call on channel 2 (right), transcribes each channel on its own, labels them `You`/`Others`, and merges them into one time-ordered transcript. Overlapping speech survives, because the two voices never share a track. A single **mixed** file can't separate your voice — everyone is `Speaker N` — so `--speaker-mode source` on a mono (or more-than-two-channel) file is a **usage error**, the same refusal the live path gives when only one source is captured, rather than silently unlabeled output. `--speaker-mode auto` keeps diarizing the whole recording acoustically whatever its channel count: only `source` states that the channels *are* the two capture sources, and an ordinary stereo recording is two microphones in the same room, not two sides of a call. Per-channel attribution still needs the offline pipeline to find each channel's speech spans, so like every other `-i --speakers` run it is Apple-Silicon-first; the platform-free guarantee in (a) is about live capture, where segmentation is already running.

**d) Separate-track audio output (`--tracks mixed|stereo`).** The same separation is available in the *recording*, not only in the transcript — and independently of `--speakers`. `--tracks stereo` writes the single `-a` file with the **microphone on channel 0 (left) and the system/app audio on channel 1 (right)** instead of summing them, so the separation survives the recording and a later `hark -i` (or any other tool) can still tell the two sides apart. `mixed` is the default and is byte-for-byte today's output. `stereo` needs the two sources (`--mix` with `--system`/`--app`/`--exclude-app`) and both channels; either missing is a usage error rather than a silent downgrade. It describes the `-a` file, so a run without one has nothing to lay out and ignores the setting — a `tracks stereo` left in the config or the environment never breaks a transcript-only capture. Each source is folded to mono before it is placed on its channel, so genuinely stereo system audio is downmixed — the cost of keeping the two sides apart in one file. Also `$HARK_TRACKS`, config `tracks`, and the remote-control `tracks` field. Everything else is unchanged: `--duration` trims both channels at the same instant, a paused interval gaps both together, `--split` chunks the interleaved stream, and a write the file cannot take ends the run with the same I/O error the mixed stream raises.
...
**Models & platform.**
  - Diarization/VAD CoreML models are managed through `hark models` (engine-tagged, e.g. `fluidaudio:diarizer`, `fluidaudio:vad`, `fluidaudio:streaming-asr`) and FluidAudio's own cache (`~/Library/Application Support/FluidAudio/Models`); `hark models list` shows them. They download from Hugging Face on first use (opt-in network, then fully local) — consistent with the `whisperkit`/`parakeet` engines and §7 Security & Privacy.
  - Acoustic diarization and VAD are **Apple-Silicon-first** (runtime-gated with a clear error on Intel, like `whisperkit`/`parakeet`); every `-i --speakers` run goes through the offline diarizer, including the per-channel `source` mode of (c), which uses it to find each channel's speech. **Live source attribution (a) has no such requirement** — it is pure stream routing and works everywhere `--mix` works, including headless. No new TCC permission is required (same captured audio).
...
- `--interactive` runs a **live capture** in a minimal terminal UI on the controlling terminal: a status header (§6.8), the live transcript as it is produced (with speaker labels when `--speakers` is active), and status lines for control hints and pause/resume/stop notices (printed on stderr so they don't fight the transcript on stdout). **Alone** it captures immediately. **With `--remote-control`** (combined mode, below) the TUI is up immediately but capture waits for `POST /start`.
- **Controls (single keypress):** **space** toggles pause/resume; **m** toggles microphone mute/unmute; **y** yanks (copies) the transcript so far to the system clipboard; **Enter** finishes the capture; any `shortcut.<k>` binding runs its command (see below). In one-shot `--interactive`, Enter and Ctrl-C both end the process (capture is finalised so every output file remains playable). In combined mode Enter only stops the session (see below); Ctrl-C quits. The **m** control is shown in the hint line only when a microphone is part of the capture; bound shortcut keys are listed on the hint line when configured.
...
- **Shortcuts (`shortcut.<k>`):** user-defined single keys run a pre-configured shell command against the live transcript. Bindings are scalar keys in `~/.hark/config.json` of the form `"shortcut.w": "…"`, set with `hark config set shortcut.w '…'`. They are **not** members of the typed settings registry (no `ConfigKey`, no `$HARK_*` counterpart, no flag); `hark config show` lists them and `unset` removes them.
  - **Valid keys:** `k` is one letter or digit (`[a-z0-9]`), case-insensitive, stored lowercase. Built-in controls cannot be rebound: **m** and **y** are rejected on set/load; space and Enter are not representable as `shortcut.<k>`. A multi-character suffix (`shortcut.summarize`) or an empty command is a usage error.
  - **On keypress:** if the live caption log is empty, print `hark: nothing to pass yet` and do not spawn (same idea as yank). Otherwise snapshot that log — the same plain text yank copies, including speaker labels when `--speakers` is active, **not** the on-disk `-t` file (which may be absent or `.srt`/`.json`) — to a temp file; spawn `/bin/sh -c <command>` with `$HARK_TRANSCRIPT` set to that path on the child only; do not wait. Child stdin, stdout, and stderr are redirected to `/dev/null` so the command cannot steal the TTY or interleave with captions. A pipeline *inside* the command (e.g. `fabric-ai … "$HARK_TRANSCRIPT" | pbcopy`) still runs inside `sh`. Print a one-line stderr ack (`hark: shortcut.w`); command output is never printed by Hark.
  - **Lifecycle:** fire-and-forget and re-entrant (a second press spawns another process). The temp file is deleted when that command exits. The controls hint lists bound keys. Shortcuts work while paused. They are **interactive-only** (not on the remote-control HTTP API; they do run in combined TUI mode). Under `--diarize-engine offline` the live log is empty until capture ends, so shortcuts no-op until then (same limitation as yank).
- **Combined with `--remote-control` (Feature 25):** the process is a long-lived TUI agent. It binds the HTTP API (§6.10) and takes over the terminal, but **does not capture until `POST /start`**. Launch capture/transcription flags are session defaults (same as a headless agent). Keyboard and HTTP share one `CaptureControl` / one session.
  - **Idle:** no capture; stderr notes that it is waiting; the hint lists quit + any shortcuts. **Enter** is a no-op. **space** / **m** notice that nothing is recording. **y** / shortcuts see an empty log until the first session (and again after a start that has produced no captions yet).
  - **Recording:** `POST /start` begins capture; live captions stream on stdout; space/m/y/shortcuts behave as in one-shot interactive; `POST /pause`/`/resume`/`/mute`/`/unmute`/`/stop` work on the same session. A second `POST /start` is `409`.
  - **Enter** = `POST /stop`: finalise outputs, return to idle, **keep** the caption log so yank/shortcuts can still recap that meeting.
  - **Next `POST /start`:** clear the caption log (meetings do not mix), then record.
  - **Ctrl-C / SIGTERM:** finalise if recording, restore the terminal, exit the process.
  - Headless `brew services` stays `--remote-control` only (no TTY → no TUI).
- **Requirements & exclusivity:** requires a controlling TTY (clear usage error otherwise). It is **mutually exclusive with stdout outputs** (`-a -`, `-t -`, `--raw`) because the UI owns the terminal. It **may** be combined with `--remote-control` (Feature 25). The live transcript is **always rendered in the UI** (on stdout) as it is produced — whether or not `-t` is named — and naming `-t FILE`/`-a FILE` **additionally persists** the transcript/audio to those files **at the same time** (the on-screen captions are plain text regardless of the file's format). Combined-mode outputs come from each `POST /start` body (over launch defaults), not from a single launch `-a`/`-t`.
...
- **Endpoints — flat control verbs (one active session):**
  - `GET /status` — agent liveness, version, bind address, and the current session's state (`idle`/`recording`/`paused`), `muted` flag, elapsed time, and output paths, plus `callAudio` (whether the system-audio side is still being heard — M8.2, docs/remote-control.md) on the captures that watch it.
  - `POST /start` — begin recording; an optional JSON body mirrors the CLI flags/outputs over the launch defaults (e.g. `{ "transcript": "notes.txt", "audio": "rec.m4a", "system": true }`), plus `muted` to begin with the mic muted. Returns the new state + resolved output paths. **The answer waits for the capture to be open**, so a client that starts talking the moment it sees the `201` is heard from its first word; a start that fails on the way up returns its own error instead of a `201` for a recording that never happened. Both the answer and `GET /status` carry `capturing`. The wait is bounded by `$HARK_START_TIMEOUT` (default 60 s) — a first-ever model download can outlast any sensible wait — after which the answer says `capturing: false` and the client watches `GET /status` for it to turn true.
  - `POST /pause` · `POST /resume` — pause/resume the active recording (pause uses the §6.9 gap semantics).
  - `POST /mute` · `POST /unmute` — mute/unmute the **microphone** of the active recording (§6.9 mute semantics: silences **only** the mic, the timeline is preserved — distinct from pause; orthogonal to `state`; idempotent). Requires a microphone in the capture (mic-only or `--mix`); on a system/app capture with no mic they return `422`. The transcript yank (`y`, §6.9) and interactive shortcuts (`shortcut.<k>`, §6.9) are **not** exposed — the API never serves transcript content and never runs user commands.
  - `POST /stop` — stop and finalise the active recording. In combined mode this is the same as **Enter**: return to idle, keep the caption log.
...
- **Security:** the listener is **loopback-only by default**, consistent with §7 "no external network calls by default" (it accepts local connections; it makes none). Binding to a **non-loopback** interface is explicit opt-in and **requires a bearer token** (`$HARK_REMOTE_TOKEN`, sent as `Authorization: Bearer …`); the agent **refuses to bind** to a non-loopback address without one. `--remote-control` is mutually exclusive with `-i` (it is an agent, not a file-transcribe mode). It **may** be combined with `--interactive` (Feature 25); it is still mutually exclusive with stdout (`-`) streams when the TUI is on.
...
|----------|-------------|
| **Performance** | Recording must use < 3% CPU on an Apple Silicon Mac (16 kHz mono); buffering delays < 100 ms. Live diarization (streaming) must keep up with real time (RTF < 1) on Apple Silicon and emit speaker labels close to runtime — a tentative label within ~1 s and a finalized one within ~2 s of a turn. Offline (`-i`) diarization is bounded by file length, not interactive. Source attribution (§6.7a) adds negligible overhead. Interactive (§6.9) and remote-control (§6.10) commands must take effect promptly — a control request is acknowledged within ~100 ms and pause/resume takes hold within one capture buffer. `POST /start` is the one exception: its answer is deliberately held until the capture is open (§6.10), so it is bounded by `$HARK_START_TIMEOUT` (default 60 s), not by the ack budget. Spawning a `shortcut.<k>` command must not stall the capture thread or the key-reader; the ack is the same ~100 ms budget, after which the child runs detached. |
...
- **Remote-control responsiveness:** API control commands (stop/pause/resume/mute/unmute) round-trip in < 200 ms on loopback, and the shipped Tampermonkey Google-Meet recipe records + names a call end-to-end without manual steps. `POST /start` is exempt: it answers once the capture is really open, which means the sources are started and any cold recognizer model is loaded (12.7 s measured on a first streamed call after a reboot), bounded by `$HARK_START_TIMEOUT` (default 60 s), after which it answers `capturing: false` anyway. Combined with `--interactive`, keyboard and HTTP stay within the same ~100 ms ack budget and must not stall captions.
- **Interactive-shortcut responsiveness:** pressing a `shortcut.<k>` key acknowledges within ~100 ms and does not drop audio or stall live captions; the bound command runs detached.
...
| **M8.2 – Dead-tap recovery** | Silent-tap watchdog (docs/reference.md#interruptions): a system tap that dies while a microphone keeps the buffers coming is found by probing with a throwaway tap, rebuilt into the same recording with a backoff and a per-stretch cap (`$HARK_TAP_SILENCE_SECONDS`), and reported as `callAudio` in `GET /status` | Post-M8 | M8, M8.1 |
| **Post-MVP** | Scheduled/unattended launchd daemon, cross-host/authenticated remote control, cloud backends, configuration profiles, named speaker identification (voiceprints), overlapping-speech handling, MP4/ID3 metadata | Ongoing | M5, M8 |
...
8. ~~**Separate-track audio output:** Whether to expose the internal mic/system separation as user-facing audio output (e.g. dual files or L/R channels), beyond its use for transcript attribution.~~ → **Resolved into scope as L/R channels (§6.7d):** `--tracks mixed|stereo`, one file, microphone left and system audio right, `mixed` unchanged as the default. L/R was chosen over dual files because it keeps the one-capture-one-artifact model of `-a` (one `--if-exists` decision, one `--split` series, one thing to hand to another tool) and because every writable container already carries two channels — while `ffmpeg -filter_complex channelsplit` recovers the two files from it in one command, the reverse being much less convenient. **Still open:** dual-file output for the cases L/R cannot serve — a genuinely stereo system signal is downmixed to mono by the fold, and more than two sources would not fit at all.
