...
- **Remote-control responsiveness:** API control commands (start/stop/pause/resume) round-trip in < 200 ms on loopback, and the shipped Tampermonkey Google-Meet recipe records + names a call end-to-end without manual steps. Combined with `--interactive`, keyboard and HTTP stay within the same ~100 ms ack budget and must not stall captions.
