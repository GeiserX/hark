...
- [x] `examples/README.md` indexes the recipes (table + Install + Prerequisites incl. system-audio TCC → docs/permissions.md) and `README.md` links to it; `hark-meeting` = interactive `--system --mix --speakers` → `<date>-slug.{mp3,txt}` then `fabric-ai -p <pattern>` → `.md`, where the pattern is chosen post-recording via an optional `fzf` picker (preview = `fabric-ai --readpattern`; `$FABRIC_PATTERN` skips it, `summarize_meeting` is the fallback when fzf is absent/non-tty/cancelled); `hark-note` (mic memo), `hark-dictate` (mic → `pbcopy`)
